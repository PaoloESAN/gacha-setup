...
class HYV_OT_PrepareForNewCharacter(Operator, CustomOperatorProperties):
    '''Organizes current character into a collection and prepares scene for a new character import'''
    bl_idname = 'hoyoverse.prepare_for_new_character'
    bl_label = 'HoYoverse: Prepare for New Character'
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        scene = context.scene

        # 1. Find what number to use for the new collection name
        existing_character_collections = [
            col for col in bpy.data.collections 
            if col.name.startswith('Character_')
        ]
        next_number = len(existing_character_collections) + 1

        # Try to detect a character name from the armature
        armatures = [obj for obj in scene.objects if obj.type == 'ARMATURE']
        if armatures:
            character_label = armatures[0].name
        else:
            character_label = f'{next_number:02d}'

        collection_name = f'Character_{character_label}'

        # Ensure unique name
        base_name = collection_name
        counter = 1
        while bpy.data.collections.get(collection_name):
            collection_name = f'{base_name}_{counter:02d}'
            counter += 1

        # 2. Create a new collection for the existing character
        new_collection = bpy.data.collections.new(collection_name)
        scene.collection.children.link(new_collection)

        # 3. Move ALL objects currently in the scene's root collection to the new collection
        objects_to_move = list(scene.collection.objects)
        for obj in objects_to_move:
            try:
                new_collection.objects.link(obj)
                scene.collection.objects.unlink(obj)
            except RuntimeError:
                pass  # object may already be in the collection

        # Also move objects from any unnamed/default collections
        # (but not from other Character_ collections)
        for col in list(scene.collection.children):
            if col == new_collection:
                continue
            if col.name.startswith('Character_'):
                continue
            # Move this entire collection under the new character collection
            try:
        ...
        # 4. Hide the old character collection in viewport
        # We need to access the view layer's layer collection to toggle visibility
        view_layer = context.view_layer
        self._set_collection_exclude(view_layer.layer_collection, collection_name, exclude=False)
        self._set_collection_hidden(view_layer.layer_collection, collection_name, hidden=True)

        # 5. Clear the addon cache so the next import starts fresh
        from setup_wizard.import_order import clear_cache
        clear_cache(self.game_type)

        # 6. Make Materials and Node Groups unique to this character
        suffix = f'_{character_label}'
        visited_trees = set()
            ...
            if obj.type == 'MESH':
                for slot in obj.material_slots:
                    ...
                    if mat and mat.use_nodes:
                        if suffix not in mat.name:
                            mat.name = f"{mat.name}{suffix}"
                        if mat.node_tree:
        ...
        self.report({'INFO'}, f'Previous character moved to collection "{collection_name}" and hidden. Scene is ready for a new character!')
        return {'FINISHED'}

    def _rename_node_groups_recursively(self, node_tree, suffix, visited):
        for node in node_tree.nodes:
            if node.type == 'GROUP' and node.node_tree:
                group_tree = node.node_tree
                # Keep track using the original or curruent name
                if group_tree.name not in visited:
                    visited.add(group_tree.name)
                    
                    if suffix not in group_tree.name:
                        group_tree.name = f"{group_tree.name}{suffix}"
                        
                    # Recursively rename groups inside THIS node group
                    self._rename_node_groups_recursively(group_tree, suffix, visited)

    def _set_collection_hidden(self, layer_collection, collection_name, hidden=True):
        """Recursively find and hide a collection in the view layer."""
        for child in layer_collection.children:
            if child.name == collection_name:
                child.hide_viewport = hidden
                return True
            if self._set_collection_hidden(child, collection_name, hidden):
                return True
        return False

    def _set_collection_exclude(self, layer_collection, collection_name, exclude=False):
        """Recursively find and set exclude on a collection in the view layer."""
        for child in layer_collection.children:
            if child.name == collection_name:
                child.exclude = exclude
                return True
            if self._set_collection_exclude(child, collection_name, exclude):
                return True
        return False
