...
def rig_character(
        ...
        for b_info in sec_bones:
            ...
            if b_name not in rig_edit_bones:
                ...
                if parent_name:
                    ...
                    target_p = rig_edit_bones.get(mapped_p) or rig_edit_bones.get(parent_name)

                if not target_p:
                    nl = b_name.lower()
                    if any(k in nl for k in ["hair", "head", "ear", "horn", "ring"]):
                        target_p = rig_edit_bones.get("DEF-spine.006") or rig_edit_bones.get("head")
                    elif any(k in nl for k in ["qun", "skirt", "tail", "pelvis"]):
                        target_p = rig_edit_bones.get("DEF-spine") or rig_edit_bones.get("hips")
                    elif any(k in nl for k in ["xiu", "sleeve", "arm", "elbow"]):
                        if ".r" in nl or "_r_" in nl or "_r" in nl:
                            target_p = rig_edit_bones.get("DEF-forearm.R") or rig_edit_bones.get("DEF-upper_arm.R")
                        else:
                            target_p = rig_edit_bones.get("DEF-forearm.L") or rig_edit_bones.get("DEF-upper_arm.L")
                    elif any(k in nl for k in ["cloth", "sce"]):
                        target_p = rig_edit_bones.get("DEF-spine.003") or rig_edit_bones.get("chest")
                    elif "thigh" in nl:
                        target_p = rig_edit_bones.get("DEF-thigh.R" if ".r" in nl else "DEF-thigh.L")
                    elif "calf" in nl:
                        target_p = rig_edit_bones.get("DEF-shin.R" if ".r" in nl else "DEF-shin.L")
    ...
    print("NTE Rigify generation complete.\n")
